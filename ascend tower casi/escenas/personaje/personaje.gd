...
@export var barra_salto: Control
# --- Muerte ---
...
func _physics_process(delta: float) -> void:
	...
	# --- BARRA DE SALTO ---
	if barra_salto:
		barra_salto.visible = jump_charge > 0.01
		barra_salto.update_carga(jump_charge)
func _jump(delta: float) -> void:
	...
	if Input.is_action_pressed("saltar"):
		jump_charge = move_toward(jump_charge, 1.0, delta * 3.0)
	else:
		jump_charge = move_toward(jump_charge, 0.0, delta * 3.0)
